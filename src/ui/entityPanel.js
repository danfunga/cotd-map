...
class EntityPanel {
    ...
    compareEntity(a, b) {
        ...
        const labelA = this.deps.entityManager.label(a);
        const labelB = this.deps.entityManager.label(b);
  ...
  `;
    ...
    getOrCreateEntityRow(entity) {
        const key = this.deps.entityManager.entityKey(entity);
  ...
  `;
        ...
        rowUi.countToggle.addEventListener("click", (event) => {
            event.stopPropagation();
            const keyByEntity = this.deps.entityManager.entityKey(entity);
    ...
    updateEntityRow(rowUi, entity) {
        ...
        rowUi.nameEl.textContent = this.deps.entityManager.label(entity);
    ...
    updateGroupHeaderState(category) {
        ...
        const allActive = groupItems.length > 0 && groupItems.every((ent) => this.deps.entityManager.isEntityActive(ent));
        ...
        const caughtCount = totalEntries.filter((entry) => this.deps.entityManager.isCaught(entry)).length;
        ui.infoEl.textContent = String(caughtCount) + "/" + String(totalEntries.length);
        const activeCount = groupItems.filter((entry) => this.deps.entityManager.isEntityActive(entry)).length;
    ...
    handleGroupShowHideToggle(category) {
        ...
        const allActive = group.length > 0 && group.every((ent) => this.deps.entityManager.isEntityActive(ent));
        group.forEach((ent) => {
            const key = this.deps.entityManager.entityKey(ent);
    ...
    handleShowAllToggle() {
        this.deps.entityManager.resetActiveEntitiesForMap(state.lastFilteredEntities);
        this.deps.saveAndRender(true);
    }
    ...
    handleHideAllToggle() {
        ...
        state.lastFilteredEntities.forEach((e) => {
            if (e.category === "monster") state.selection.activeEntityKeys.add(this.deps.entityManager.entityKey(e));
        });
        state.selection.initializedActiveMapIds.add(state.currentMapId);
        this.deps.saveAndRender(true);
    }
    ...
    handleShowUncaughtOnly() {
        let uncaughtEntry = state.lastFilteredEntities.filter(entry => !this.deps.entityManager.isCaught(entry));
        this.deps.entityManager.resetActiveEntitiesForMap(uncaughtEntry);
        state.lastFilteredEntities.forEach((e) => {
            if (e.category === "monster") state.selection.activeEntityKeys.add(this.deps.entityManager.entityKey(e));
        });
        this.deps.saveAndRender(true);
